      ...
      <box width={contentWidth()} flexGrow={1} flexDirection="column" paddingTop={1} paddingBottom={1} gap={1}>
        ...
        <Show when={showFooter()}>
          <box flexDirection="column" alignItems="center" flexShrink={0}>
            ...
            <text fg={colors.muted}>marid {InstallationVersion}</text>
          </box>
        </Show>
      </box>
    </box>
...
function buildIssueURL(message: string, stack: string) {
  // Opens a new issue on the Marid repo pre-filled with title/body. The extra
  // os/terminal/reproduce keys populate a bug-report form template when present and
  // are harmlessly ignored otherwise.
  const url = new URL("https://github.com/A-H-911/marid/issues/new")
  ...
  url.searchParams.set(
    "reproduce",
    "Reported automatically from the marid crash screen. If you can, describe what you were doing when it crashed.",
  )
  ...
  const marker = "\n... (truncated)"
  const head = `The marid TUI crashed with an unexpected error.\n\n**Error:** ${message}\n\n**Stack trace:**\n`
