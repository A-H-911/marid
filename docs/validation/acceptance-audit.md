...
| AC-019 | Cross-client bidirectional mirroring + view-via-binding/act-via-ownership | Partial | **WBS-6.3 + 6.4 properties done (mechanism + cross-surface permission verified).** WBS-6.3: durable `BindingStore` + binding-aware `isVisible` (`owns ∪ bound`) at the `/event` site (additive, zero upstream edit; `event-filter.ts` untouched); `mirroring.test.ts` proves mirror-in **and** mirror-out, explicit-attach scope (unattached invisible), **act-via-ownership** deny (INV-001), blast-radius no-op, registry-fault degradation (RISK-024). WBS-6.4: **first-responder-wins / no-double-approve** proven as a server invariant (`Permission.reply` consumes atomically → second reply `NotFoundError`, `permission/next.test.ts`); **permission.asked mirrors to a bound surface** (view; `mirroring.test.ts`); concurrency = EXP-001 (one-Runner join/steer, act limited to owners). **Remaining for Met:** operator-reachable **attach** (admin-gated HTTP endpoint = WBS-6.1, AC-024/OpenAPI) + live end-to-end path (`/global/event` routing = WBS-6.1; attach-triggers-reconnect = WBS-6.5). TEST-SYNC slices green | post-MVP · PH-6 |
