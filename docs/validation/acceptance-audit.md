...
| AC-019 | Cross-client bidirectional mirroring + view-via-binding/act-via-ownership | Partial | **WBS-6.3 done (mirroring mechanism).** Durable `BindingStore` + binding-aware `isVisible` (`owns ∪ bound`) at the `/event` filter site — additive, zero upstream edit (`event-filter.ts` untouched). `mirroring.test.ts` (production successor to EXP-008) proves: mirror-in **and** mirror-out (bidirectional), explicit-attach scope (unattached session invisible), **act-via-ownership** deny of a bound-but-not-owner reply/prompt (INV-001), blast-radius no-op for plain clients, registry-fault degradation (RISK-024). **Remaining for Met:** cross-surface permission surfacing (first-responder-wins, no double-approve) = WBS-6.4. TEST-SYNC mirroring slice green | post-MVP · PH-6 |
