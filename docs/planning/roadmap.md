...
| PH-8 Isolation & deep rebrand ⏳ | Total **DATA** isolation from a co-installed OpenCode (dirs/config; `OPENCODE_*` env kept) + deep rebrand (agent identity, TUI, web) + upstream sync — ADR-0018 | **MS-009 (not-started)**: AC-025..031 Met (data isolation, env-pierce disclosure, no update popup, agent identity, TUI branding, web branding, migration) + all 6 reported v0.2.0 issues map to a passing check; v0.3.0 released | MS-007 (post-release; **ADR-0018 + DEC-022..027 Approved 2026-07-13**; WBS-8.0 merged #53, WBS-8.1 sync at merge gate #54) |
