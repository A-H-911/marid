...
| WBS-6.2 | **Full Telegram experience** (fix-in-place) — Markdown via `telegramify-markdown`; **files both ways** (inbound `resolveDownloadUrl`→`FilePartInput` + outbound send); **whitelisted slash commands**; inline keyboards; multi-part separation | Full round-trip incl. files+slash+inline-kbd; INV-002/004 respected | ADR-0009, DEC-014, FR-046/048/049, DEP-012 | not-started |
