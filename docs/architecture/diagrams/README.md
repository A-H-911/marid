...
| Colour | Meaning |
|---|---|
| 🟥 **red** | `@marid/gateway` — the **Marid Gateway** (API/ingress: bearer-token auth / scopes / `/marid/*` / `owns ∪ bound` mirroring; `marid-auth` is its auth module, ADR-0011) |
| 🟧 **orange** | `marid-telegram` — the **channel** gateway, a *client* of `@marid/gateway` (separate process, untrusted ingress) |
...
Marid adds only four things as **new packages speaking existing interfaces** (`marid-gateway`,
...
| 18 | [Contributor workflow](Marid/18-contributor-workflow.png) | swimlane | The Marid dev loop across You / Local repo / GitHub-CI lanes (install → dev → check → test → generate → **PR to `develop`** → 20 checks → operator merge). Uses the `corporate` (cool blue/slate/teal) theme to avoid colliding with the Marid red/orange palette. |
...
| 20 | [Gateway & mirroring](Marid/20-gateway-mirroring.png) | flowchart | **PH-6**: the **Marid Gateway** (`@marid/gateway`; `/marid/*` binding routes + `owns ∪ bound` SSE filter) + **`@marid/channel-client`** (subscribe / reconnect / re-fetch) + **cross-surface mirroring** + Telegram tool calling over the sync `/session/{id}/message` route — the channel platform on top of the Gate-5 design |
| 21 | [WhatsApp channel](Marid/21-whatsapp-channel.png) | flowchart | **PH-7**: `@marid/whatsapp` as a second channel on the ADR-0012 registry — reaching WhatsApp **only** through the digest-pinned **WAHA sidecar**, **outbound-only** (WS events in / HTTP out), reusing `@marid/channel-client` unchanged, with `APPROVE <token>` text permission (ADR-0015) and INV-001 server-enforced by `@marid/gateway` |
