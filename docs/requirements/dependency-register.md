...
| DEP-013 | `WAHA` (devlikeapro) image `devlikeapro/waha:noweb-2026.7.1@sha256:8717e9a6…` — **digest-pinned** (repo's first). Baileys (WhiskeySockets, MIT) = **reference-only, not depended on** | Unofficial WhatsApp client for `@marid/whatsapp` (FR-047, ADR-0010) — reached via an operator-run WAHA sidecar, **outbound-only** | Verified 2026-07-17 (GATE-0/EXP-006): WAHA-NOWEB **Core (free)** exposes WS event mode + sendText + media + editMessage + presence; `@marid/whatsapp` pulls **zero WhatsApp npm dependency** (Bun `fetch`+`WebSocket`); the pinned digest is the RISK-014 trust anchor + the GATE-0 fixture baseline. Baileys-direct documented-not-built (DEC-015) | RISK-013 (ToS/ban), RISK-014 (supply-chain — **mitigated**, no npm dep) | Accepted (WAHA-only) |
